...
| 法术储存戒指 Spell-Storing Ring | 本次长休使用次数未消耗；已储存虚假生命 False Life（1 环；施放后获得 2d4 + 4 临时生命值） |
...
| 法术储存戒指 Spell-Storing Ring | 1 枚 | KEN 击败马特委托所称的黑暗势力首领、完成部分委托后，马特在旅馆会面时交付给 Nym；蛋蛋拒绝该戒指。这就是既有的同一枚戒指，并非第二枚。可储存任意 1 环法师法术，每次长休可使用 1 次；当前储存虚假生命 False Life。该法术的材料为一滴酒精，不属于贵重耗材 |
